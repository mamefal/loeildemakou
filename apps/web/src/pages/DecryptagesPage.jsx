...
const DecryptagesPage = () => {
  const [articles, setArticles] = useState([]);
  const [loading, setLoading] = useState(true);
  const [page, setPage] = useState(1);
  const [totalPages, setTotalPages] = useState(1);
  
  const categorySlug = 'decryptages';
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        console.error("Error fetching decryptages:", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background">
      <Helmet>
        <title>Décryptages - L'œil de Makou</title>
        <meta name="description" content="Analyses approfondies des tendances de fond et phénomènes de société liés à la mode." />
      </Helmet>
      ...
      <main className="flex-grow py-16 md:py-24 container mx-auto px-4 max-w-7xl">
        <header className="mb-16 text-center max-w-3xl mx-auto">
          <h1 className="font-serif text-4xl md:text-5xl lg:text-6xl font-bold text-primary mb-6">Décryptages</h1>
          <p className="text-accent font-medium italic text-xl mb-6">Pourquoi ce sujet est important ?</p>
          <p className="text-muted-foreground text-lg leading-relaxed">
            Plongée au cœur des phénomènes de société, des évolutions esthétiques et des mouvements culturels qui traversent l'industrie de la mode.
          </p>
        </header>
...
export default DecryptagesPage;